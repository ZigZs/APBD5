...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var test = emps.SelectMany(e => grades
                .Where(g => g.Losal <= e.Sal && e.Sal <= g.Hisal),
            (e, g) => new { e.Sal, g.Grade }
        );
        var result = depts.Join(
            emps.SelectMany(e => grades
                    .Where(g => g.Losal <= e.Sal && e.Sal <= g.Hisal),
                (e2, g2) => new {e2, g2.Grade}
            ),
            d => d.DeptNo,
            e => e.e2.DeptNo,
            (d, e) => new { EName = e.e2.EName, DName = d.DName, Grade = e.Grade }
        ).ToList();
